...
| Intégrité | Unicités `presence(session_id, etudiant_id)`, `exercice(session_id, etudiant_id)`, `relecture(exercice_id, relecteur_id)` (V2 : deux relecteurs par exercice) ; `CHECK` note 0–20, statut, source |
